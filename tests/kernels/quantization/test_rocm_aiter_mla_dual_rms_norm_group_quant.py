...
un-fused pair: rocm_aiter_rmsnorm_fp8_group_quant (Triton) for q + reference RMSNorm for kv."""
import pytest
import torch
...
@pytest.mark.skipif(
    ...
    or not rocm_aiter_ops.is_fused_qk_rmsnorm_group_quant_available(),
    reason="ROCm AITER >= 0.1.20 only",
)
...
def test_fused_mla_dual_rms_norm_group_quant_matches_unfused(transpose_scale):
    with set_current_vllm_config(VllmConfig()):
        ...
        q_ref, s_ref = torch.ops.vllm.rocm_aiter_rmsnorm_fp8_group_quant(
            q_c, qw, eps, G, transpose_scale
        )
        xf = kv_c.float()
        kv_ref = (
