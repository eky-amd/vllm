...
class MLADualRMSGroupQuantPattern(
    ...
    ``transpose_scale`` is carried through so the CK b-preshuffle GEMM keeps
    receiving column-major activation scales.
    """
...
class MLADualRMSNormFusionPass(VllmFusionPatternMatcherPass):
    ...
    def __init__(self, config: VllmConfig) -> None:
        ...
        for epsilon in [1e-5, 1e-6]:
            ...
            if rocm_aiter_ops.is_fused_qk_rmsnorm_group_quant_available():
                # fp8 block-scale (group quant) path, e.g. DeepSeek; the AITER
                # kernel exists from v0.1.20.
                for transpose_scale in (False, True):
                    self.register(
