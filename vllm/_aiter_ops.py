...
def _aiter_fused_ar_rms_per_group_quant_transposed(
    ...
    """aiter's ``fused_ar_rms_per_group_quant`` launcher with the C++
    ``transpose_scale`` flag (exposed by the pybind but not by the Python
    launcher): the scale buffer keeps shape [M, G] but holds the bytes of
    scale.T, the layout the CK b-preshuffle blockscale GEMM consumes."""
    import aiter as aiter_ops

    K = inp.shape[-1]
    res_out = torch.empty_like(inp)
    out = torch.empty(inp.shape, dtype=FP8_DTYPE, device=inp.device)
    scale_out = torch.empty(
        inp.shape[:-1] + (K // group_size,), dtype=torch.float32, device=inp.device
    )
    registered = torch.cuda.is_current_stream_capturing()
    reg = 0 if registered else ca._pool["input"].data_ptr
    reg_bytes = 0 if registered else ca._pool["input"].max_size
    aiter_ops.fused_allreduce_rmsnorm_quant_per_group(
        ca._ptr,
        inp,
        res_inp,
        res_out,
...
def _fused_mla_dual_rms_norm_group_quant_impl(
    ...
    Group-quant sibling of ``fused_mla_dual_rms_norm_per_token_quant`` for the
    fp8 block-scale path (DeepSeek): the q latent is RMS-normed and group-
    quantized (one scale per ``group_size`` elements, optionally written in the
    transposed layout the CK b-preshuffle GEMM consumes) while the kv latent is
    RMS-normed to bf16 for the KV cache / attention. One HIP launch replaces the
    Triton rms+group-quant kernel and the standalone kv RMSNorm.
    Requires AITER >= 0.1.20 (fused_qk_rmsnorm_group_quant).
    """
...
class rocm_aiter_ops:
    ...
    @staticmethod
    def is_fused_qk_rmsnorm_group_quant_available() -> bool:
        """AITER >= 0.1.20 ships the fused q/kv RMSNorm + group-quant kernel."""
        try:
